...
- (id)initWithFrame:(CGRect)frame{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        [self addLinePosi_Y:ViewHeight(self) - 0.5];
        _selectedIndex = -1;
...
- (void)setSourceArray:(NSArray *)sourceArray{
    ...
    for (int i = 0; i < [_sourceArray count]; i++) {
        KLandscapeLabel *lable =[[KLandscapeLabel alloc]initWithFrame:CGRectMake(i * VIEW_WIDTH, 0, VIEW_WIDTH, ViewHeight(self)) ];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (offset_x > _lastOffset_x) {
        ...
        //向右滑动
        if (fabs((int)offset_x % (int)VIEW_WIDTH) < VIEW_WIDTH / 2.0f) {
            [self resetSelectColor:((int)offset_x / (int)VIEW_WIDTH)];
        }else {
            [self resetSelectColor:((int)offset_x / (int)VIEW_WIDTH + 1)];
        }
...
- (void)resetSelectColor:(NSInteger)index{
    ...
    thisLabel.textColor = [UIColor colorWithHexString:@"#05c148"];
